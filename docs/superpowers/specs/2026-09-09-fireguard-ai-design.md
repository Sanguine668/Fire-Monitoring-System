...
- 文档版本：v1.0（设计确认版）
- 日期：2026-09-09
- 关联文档：《项目介绍与任务分工.md》（v0.2，人员 A/B/C/D 角色定义见该文）
...
- 演示机固定为成员 A 的 NVIDIA 笔记本；AI 训练、后端与演示构建在该机完成；
- FastAPI 启动监听 `0.0.0.0`，Windows 防火墙放行所需端口；
- 手机热点演示拓扑：笔记本连接手机热点 → 手机端 IP Webcam 提供画面 → worker 拉取手机地址；
- 前端成员（成员 B）可在自己的机器上用 Vite dev server + mock 数据先行开发，接口以 4.4/4.5 契约为准。
...
| 周次 | A 线（AI/后端：成员 A + C/D 辅助） | B 线（Vue3 前端：成员 B） |
| --- | --- | --- |

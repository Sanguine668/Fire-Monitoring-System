...
def build_charter() -> None:
    ...
    numbered(doc, [
        "蔡俊杰：系统采用分层实现，视频源统一抽象为 file/http/rtsp 三类接入，由检测任务抽帧调用自训练 YOLO 模型推理，结果经告警规则引擎判定后写入 SQLite 并通过 WebSocket 推送，前端与检测、接口层相互独立，便于替换模型与扩展视频源。",
        "李汶航：前端基于 Vue3 与 Element Plus 实现五个功能页面，通过 REST 接口获取视频源、告警与配置数据，通过 WebSocket 接收实时告警事件，通过 MJPEG 直接呈现带检测框的实时画面，形成展示、监控与管理一体的界面层。",
    ])

...
def build_charter() -> None:
    ...
    numbered(doc, [
        "蔡俊杰：系统实现了从视频源接入、AI 烟火检测、分级告警到前端展示的完整闭环，模型训练、检测链路与系统集成均已落地，下一阶段聚焦数据治理与远距离识别以提升准确率与场景适应性。",
        "李汶航：前端完成监控总览、实时监控、告警中心、视频源管理与系统设置五个页面，并与后端接口、实时画面和告警事件完成对接，能够直观展示检测结果与告警信息。",
    ])
